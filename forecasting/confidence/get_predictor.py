...
def get_predictor_model() -> ErrorPredictor:
    ...
    try:
        ...
        reg = GradientBoostingRegressor(max_depth=3, n_estimators=100)

        param_grid = {"max_depth": [2, 3, 5], "n_estimators": [25, 50, 75, 100]}

        group_map = None
        n_groups = 5

        predictor = ErrorPredictor(
            base_model=reg, stats=stat_names, param_grid=param_grid
        )
        ...
        for stat, data in stats.items():
            if isinstance(group_map, type(None)):
                ...
                player_groups = pd.DataFrame({"player": players, "group": group})
            ...
            predictor.fit_stat_model(stat, X, y, groups)
